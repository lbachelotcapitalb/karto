...
const ask = (prompt, title) =>
  execFileSync('bash', [ASK, prompt, title], { encoding: 'utf8' }).trim();   // throw si annulé/vide
...
const PROVIDERS = {
  bitwarden: (() => {
    ...
    return {
      ...
      async unlock(ask) {
        const status = JSON.parse(bw(['status']).trim());
        ...
        if (status.status !== 'unlocked' || !session) {
          const master = ask('Mot de passe maître Bitwarden (déverrouillage)', 'Bitwarden · déverrouiller').normalize('NFC');   // macOS osascript renvoie parfois du NFD → clé invalide sur passphrase accentuée
          try {
            session = execFileSync('bw', ['unlock', '--passwordenv', 'BW_MASTER', '--raw'],
              { encoding: 'utf8', env: { ...process.env, BW_MASTER: master } }).trim();
          } catch { throw new Error('Déverrouillage refusé (mot de passe maître invalide ?). Relance.'); }
          if (!session) throw new Error('Pas de session Bitwarden obtenue.');
        }
        return { session };
      },
...
const cartoPass = ask('Passphrase karto (rebuild chiffré)', 'karto · rebuild');
const r2 = spawnSync('node', [join(DIR, 'karto-sync.mjs'), 'rebuild'],
  { cwd: DIR, stdio: 'inherit', env: { ...process.env, CARTO_PASS: cartoPass } });
if (r2.status !== 0) { console.error('✗ Rebuild karto échoué (coffre OK, registre local OK).'); process.exit(1); }
