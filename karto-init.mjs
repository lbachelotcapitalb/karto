...
try {
  for (;;) {
    const a = await askHidden('  Choisis ta passphrase karto (masquée, ≥ 12) : ');
    if (a.length < 12) {
      console.log('  ✗ minimum 12 caractères.');
      console.log('    Le coffre est un fichier : qui l\'obtient l\'attaque hors ligne, sans');
      console.log('    limite de débit. Sa longueur est la seule barrière.\n');
      continue;
    }
