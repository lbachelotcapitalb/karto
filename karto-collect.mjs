...
/* ---------- navigateurs + serveurs MCP (nom + transport/scope, jamais d'env/secret) ---------- */
const browsers = (cfg.browsers || []).filter(b => existsSync(b.app)).map(b => b.name);
const mcpServers = [];
const MCP_CONFIGS = [
  { p: join(HOME, 'Library/Application Support/Claude/claude_desktop_config.json'), scope: 'claude-desktop' },
  { p: join(HOME, '.claude.json'), scope: 'claude-code' },
];
for (const { p, scope } of MCP_CONFIGS) {
  if (!existsSync(p)) continue;
  try {
    const j = JSON.parse(readFileSync(p, 'utf8'));
    for (const [name, def] of Object.entries(j.mcpServers || {})) {
      const transport = def.url ? (def.type || 'http') : 'stdio';
      const origin = def.url ? String(def.url).replace(/\?.+$/, '') : [def.command, ...(def.args || [])].join(' ').slice(0, 120);
      const seen = mcpServers.find(s => s.name === name);
      if (seen) { if (!seen.scope.includes(scope)) seen.scope.push(scope); continue; }
      mcpServers.push({ name, transport, origin, scope: [scope] });
    }
  } catch {}
}
...
(await import('./karto-sources.mjs')).touchSource(__dir, 'mac-disk');
