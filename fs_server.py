...
"""
import json, os, re, stat, mimetypes, urllib.parse
...
# CORS strict (audit 24/06) : seulement loopback avec port optionnel, host EXACT.
# Évite que http://localhost.attaquant.com ou http://localhost:9/@evil passe le startswith().
_CORS_OK = re.compile(r'^https?://(localhost|127\.0\.0\.1)(:\d{1,5})?$')


def _cors(h):
    origin = h.headers.get('Origin', '')
    if _CORS_OK.match(origin):
