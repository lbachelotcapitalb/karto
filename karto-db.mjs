...
db.exec(`
  ...
  CREATE TABLE entity (
    ...
    kind TEXT NOT NULL,         -- project|account|database|repo|host|automation|scenario|connector|webhook|ea_asset|data_asset|device|runtime|cli|launchagent|ssh_host|secret|bridge|person|agent|skill
    name TEXT NOT NULL,
...
for (const g of (cgh.actions || [])) E('automation:gha-' + slug(g.repo + '-' + g.workflow), 'automation', `GHA ${g.repo} · ${g.workflow}`, { vendor: 'GitHub', statut: g.status === 'active' ? 'Actif' : 'En pause', source: 'cloud_inventory.json', attrs: { trigger: g.trigger, does: g.does, claudeTier: g.claudeTier, obs: g.obs, chain: g.chain, repo: g.repo } });
const mk = cloud.make || {};
for (const s of (mk.scenarios || [])) { const id = E('scenario:' + slug(s.id || s.name), 'scenario', 'Make · ' + s.name, { vendor: 'Make', statut: s.active ? 'Actif' : 'En pause', source: 'cloud_inventory.json', attrs: { trigger: s.trigger, modules: s.modules, does: s.does, claudeTier: s.claudeTier, obs: s.obs, chain: s.chain } }); L(id, ref('Make') || 'account:make', 'scénario', 'cloud_inventory.json'); }
...
for (const r of (collab.repos || [])) {
  ...
  if (ents.has(repoId)) Object.assign(ents.get(repoId).attrs, openAttrs);
...
if (mi) {
  ...
  for (const m of (mi.mcpServers || [])) {
    ...
    if (hit) Object.assign(hit.attrs, { local: { transport: srv.transport, scope: srv.scope, origin: srv.origin } });
...
if (vi) {
  ...
  if (ents.has(hostId)) Object.assign(ents.get(hostId).attrs, {
...
for (const s of (srcReg.sources || [])) {
  db.prepare('INSERT OR REPLACE INTO source(id,name,method,collector,requires,cadence_days,status,last_synced,howto,note) VALUES(?,?,?,?,?,?,?,?,?,?)')
    .run(s.id, s.name || null, s.method || null, s.collector || null, s.requires || null, s.cadence_days ?? null, s.status || null, s.last_synced || null, s.howto || null, s.note || null);
}

/* ============ runs (dernier passage des automatisations — data/runs_summary.json) ============ */
// Moule prêt pour le collecteur de runs (phase 2) : merge attrs.lastRun sur l'automatisation
// dont le nom canonique contient la clé (même convention que automation_plain / gouvernance).
const runsSum = load('runs_summary.json') || {};
{
  const autoEnts = [...ents.values()].filter(e => ['automation', 'scenario', 'launchagent', 'agent'].includes(e.kind));
  let matched = 0;
  for (const r of (runsSum.runs || [])) {
    const k = canon(r.key);
    const hit = autoEnts.find(e => e.canonical.includes(k));
    if (hit) { hit.attrs.lastRun = { at: r.last_run, status: r.status, duration_s: r.duration_s, note: r.note, source: r.source }; matched++; }
  }
  if ((runsSum.runs || []).length) console.log(`  runs: ${matched}/${runsSum.runs.length} clé(s) de run rattachée(s)`);
}
...
for (const b of (buData.units || [])) {
  const id = E('bu:' + slug(b.id || b.name), 'business_unit', b.name, {
    ...
    attrs: { role: b.role, tagline: b.tagline, revenue: b.revenue, domaines: b.domaines, members: b.members,
